...
That call is the entire design. It is deliberately asymmetric: the model can _raise_ the level, since it read the actual match text and may notice that five superficially-similar hits are all solving a different problem, but it cannot lower it. If the embeddings say `Crowded` and the model says `Open`, the answer is `Crowded`. Not because the model was overruled by a special case, but because `max` does not have a branch where the smaller value wins.
