...
const spotifyLabels: Record<
  string,
  { playing: string; last: string; history: string; recent: string }
> = {
  en: {
    playing: 'Currently listening to',
    last: 'Last listened to',
    history: 'Top this month',
    recent: 'Recently played',
  },
  ru: {
    playing: 'Сейчас слушает',
    last: 'Последний раз слушал',
    history: 'Топ за месяц',
    recent: 'Недавно слушал',
  },
  az: {
    playing: 'Hazırda dinləyir',
    last: 'Son dinlədiyi',
    history: 'Bu ayın ən çox dinlənilənləri',
    recent: 'Son dinlənilənlər',
  },
};
...
export function renderSpotify(data: any) {
  ...
  const sig =
    data.track + '|' + data.playing + '|' + data.duration + '|' + (data.topTracks || []).length;
...
function _applySpotify(el: HTMLElement, data: any) {
  ...
  const trackHtml = needsMarquee
    ? '<span class="spotify-marquee-wrap"><span class="spotify-marquee">' +
      trackText +
      '   •   ' +
      trackText +
      '   •   </span></span>'
  ...
  const progress =
    data.playing && data.duration
      ...
      : '';
  const miniCover = coverUrl
    ? '<img class="spotify-mini-cover" src="' +
      _escHtml(coverUrl) +
  ...
  if (topTracks.length > 0) {
    const historyLabel =
      (data.historySource === 'recent' ? labels.recent : labels.history) || 'Top this month';
    recentHtml =
      '<div class="spotify-history" id="spotify-history"><span class="spotify-history-label">' +
      historyLabel +
      '</span>';
    for (let i = 0; i < topTracks.length; i++) {
      const r = topTracks[i];
      const rCover = r.cover
        ? '<img class="spotify-history-cover" src="' +
          _escHtml(r.cover) +
          '" alt="" width="40" height="40">'
        : '';
      recentHtml +=
        '<a href="' +
        _safeSpotifyUrl(r.url) +
        '" target="_blank" rel="noopener" class="spotify-history-item" style="--i:' +
        i +
        '">' +
        '<span class="spotify-history-idx">' +
        (i + 1) +
        '</span>' +
        rCover +
        '<span class="spotify-history-info"><span class="spotify-history-track">' +
        _escHtml(r.track) +
        '</span><span class="spotify-history-artist">' +
        _escHtml(r.artist) +
        '</span></span></a>';
    }
    recentHtml += '</div>';
  ...
  el.innerHTML =
    '<div class="spotify-inline" id="spotify-toggle">' +
    vinylHtml +
    label +
    ' <a href="' +
    safeUrl +
    ...
    '</div>' +
    progress +
    '<div class="spotify-mini-player"><a href="' +
    safeUrl +
    '" target="_blank" rel="noopener" class="spotify-mini-link">' +
    miniCover +
    '<div class="spotify-mini-info"><span class="spotify-mini-track">' +
    safeTrack +
    '</span><span class="spotify-mini-artist">' +
    safeArtist +
    '</span></div></a></div>' +
    recentHtml;
    ...
    if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      ...
      if (inlineEl) {
        ...
        while ((tnode = walker.nextNode() as Text | null)) {
          if (
            tnode.textContent?.trim() &&
            !(tnode.parentNode as Element)?.closest?.('.spotify-expand-hint')
          )
  ...
  if (toggle) {
    ...
    if (songLink && window.matchMedia('(hover: hover)').matches) {
      songLink.addEventListener('mouseenter', () => {
        el.classList.add('mini-hover');
      });
      ...
      if (mini) {
        mini.addEventListener('mouseleave', () => {
          el.classList.remove('mini-hover');
        });
      }
    }
  ...
  if (data.playing && data.duration) {
    ...
    function tickProgress() {
      ...
      const elapsed = Math.min(
        _spotifyProgress.offset + (Date.now() - _spotifyProgress.start),
        _spotifyProgress.duration,
      );
