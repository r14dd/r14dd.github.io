...
export const isRaft = (name: string): boolean =>
  name.includes("RAFT") || name.includes("Raft");
...
const isCurrent = (period: string): boolean =>
  /Present|Настоящее время|indiyə kimi/.test(period);

const stripImpactPrefix = (s: string): string =>
...
export const buildConnect = (data: I18nProfile): string => {
  return `
        ...
        <div class="connect-links">
          ...
          <a class="cta-btn" href="mailto:${data.email}"><svg class="cta-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>Email</a>
        </div>
