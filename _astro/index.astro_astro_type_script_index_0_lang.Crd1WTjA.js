const N=document.getElementById("i18n-data"),A=N?JSON.parse(N.textContent||"{}"):{},F={en:"English",ru:"Русский",az:"Azərbaycanca",es:"Español"},R=()=>{const t=window.location.hash.replace("#","");if(!t)return;const e=document.getElementById(t);e&&e.scrollIntoView({behavior:"smooth",block:"start"})};window.addEventListener("hashchange",R);window.addEventListener("load",R);const k=Array.from(document.querySelectorAll("section[id]")),B=Array.from(document.querySelectorAll(".side-nav a")),$=document.querySelector(".mobile-current"),j=()=>{const t=window.scrollY+120;let e=k[0];k.forEach(l=>{l.offsetTop<=t&&(e=l)}),window.scrollY+window.innerHeight>=document.body.scrollHeight-4&&(e=k[k.length-1]),B.forEach(l=>l.classList.remove("active"));const n=B.find(l=>l.getAttribute("href")===`#${e.id}`);if(n?.classList.add("active"),$){const l=n?.textContent??e.getAttribute("data-label")??e.id;$.textContent=l,$.classList.add("active")}const r=document.querySelector(".nav-pill"),u=document.querySelector(".side-nav");if(r&&n&&u){const l=u.getBoundingClientRect(),i=n.getBoundingClientRect();r.style.top=i.top-l.top+(i.height-26)/2+"px",r.style.opacity="1"}},H=()=>{const t=document.getElementById("scroll-progress");if(!t)return;const e=document.body.scrollHeight-window.innerHeight,n=e>0?window.scrollY/e*100:0;t.style.height=`${n}%`},P=()=>{const t=document.querySelector(".side-nav"),e=document.querySelector(".side-links");if(!t&&!e)return;const n=window.scrollY>40;t?.classList.toggle("blurred",n),e?.classList.toggle("blurred",n)};document.querySelectorAll(".side-nav a, .mobile-menu a").forEach(t=>{t.addEventListener("click",e=>{e.preventDefault();const n=t.getAttribute("href")?.replace("#","");if(!n)return;const r=document.getElementById(n);r&&(r.scrollIntoView({behavior:"smooth",block:"start"}),history.pushState(null,"",`#${n}`))})});window.addEventListener("scroll",()=>{j(),H(),P()});window.addEventListener("load",()=>{j(),H(),P()});document.querySelectorAll(".simulate-btn").forEach(t=>{t.addEventListener("click",()=>{t.closest(".project-card")?.querySelector(".sim-visual")?.classList.toggle("active")})});const Y=document.querySelector(".hamburger"),w=document.querySelector(".mobile-menu");Y?.addEventListener("click",()=>{w?.classList.toggle("open"),w?.setAttribute("aria-hidden",w.classList.contains("open")?"false":"true")});w?.querySelectorAll("a").forEach(t=>{t.addEventListener("click",()=>{w.classList.remove("open"),w.setAttribute("aria-hidden","true")})});const v=document.getElementById("lang-toggle"),f=document.getElementById("lang-menu"),I=document.getElementById("lang-current"),O=t=>`
        ...
        ${t.experience.map(e=>`
            ...
            `).join("")}
      `,X=t=>t.includes("RAFT")||t.includes("Raft")||t.includes("Hash Table")||t.includes("Distributed")||t.includes("Kademlia")||t.includes("Redis"),G=t=>t.includes("RAFT")||t.includes("Raft"),K=t=>t.includes("Hash Table")||t.includes("Distributed")||t.includes("Kademlia"),W=t=>t.includes("Redis"),_=t=>{const e=[];return t.links?.github&&e.push(`<a class="project-link" href="${t.links.github}" target="_blank" rel="noopener">${q.labels.links.github}</a>`),X(t.name)&&e.push('<button class="simulate-btn" type="button">Simulate</button>'),e.join("")},z=t=>G(t.name)?`
      ...
      `,ee=t=>{const n=(t.labels.connectTemplate||"Reach me at {email}").replace("{email}",'<a href="mailto:riadmukh@gmail.com" target="_blank" rel="noopener">riadmukh@gmail.com</a>');return`
        <h2>${t.labels.headings.connect}</h2>
        <p class="connect-text">${n}</p>
      `},te=t=>{const e=document.querySelector(".intro-name");e&&(e.setAttribute("aria-label",t),e.innerHTML="",Array.from(t).forEach((n,r)=>{const u=document.createElement("span");u.textContent=n===" "?" ":n,u.style.animationDelay=`${(.05+r*.08).toFixed(2)}s`,e.appendChild(u)}))},se=t=>{const e=A[t]||A.en;q=e,document.documentElement.lang=t,I&&(I.textContent=F[t]||"English");const n=document.querySelector("main section h1"),r=document.querySelector(".hero-links"),u=document.querySelector(".hero-cta .cta-btn"),l=document.querySelectorAll(".hero-tagline");if(n&&(n.textContent=e.hero.name),l.length){const s=e.hero.tagline.split(". "),c=s[0]?.trim()??"",x=s.slice(1).join(". ").trim();l[0].textContent=`${c}${c.endsWith(".")?"":"."}`,l[1]&&(l[1].textContent=x)}if(r){const s=r.querySelectorAll("a");s[0]&&(s[0].textContent=e.labels.links.resume,s[0].setAttribute("href",e.links.resume)),s[1]&&(s[1].textContent=e.labels.links.linkedin,s[1].setAttribute("href",e.links.linkedin)),s[2]&&(s[2].textContent=e.labels.links.github,s[2].setAttribute("href",e.links.github))}u&&(u.setAttribute("href",e.links.resume),u.innerHTML=`<svg class="cta-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4v10m0 0l-4-4m4 4l4-4M5 20h14" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"></path></svg>${e.labels.links.resume}`);const i=document.querySelector(".intro-phrase");if(i){const s=i.querySelectorAll(".lead"),c=!!e.labels.introLead,x=!!e.labels.introTail;if(s.length===0&&(c||x)){if(c){const g=document.createElement("span");g.className="lead",g.textContent=e.labels.introLead,i.insertBefore(g,i.firstChild)}if(x){const g=document.createElement("span");g.className="lead",g.textContent=e.labels.introTail,i.appendChild(g)}}else if(s.length===1){if(c&&!x)i.insertBefore(s[0],i.querySelector(".intro-name")),s[0].textContent=e.labels.introLead;else if(!c&&x)i.appendChild(s[0]),s[0].textContent=e.labels.introTail;else if(c&&x){s[0].textContent=e.labels.introLead,i.insertBefore(s[0],i.querySelector(".intro-name"));const g=document.createElement("span");g.className="lead",g.textContent=e.labels.introTail,i.appendChild(g)}}else s.length>=2&&(c?(s[0].textContent=e.labels.introLead,s[0].style.display=""):s[0].style.display="none",x?(s[s.length-1].textContent=e.labels.introTail,s[s.length-1].style.display=""):s[s.length-1].style.display="none")}te(e.hero.name);const a=document.querySelector(".side-links");if(a){const s=a.querySelectorAll("a");s[0]&&(s[0].textContent=e.labels.links.resume),s[1]&&(s[1].textContent=e.labels.links.linkedin),s[2]&&(s[2].textContent=e.labels.links.github)}const o=document.querySelectorAll(".side-nav a"),d=document.querySelectorAll(".mobile-menu a"),h=[e.labels.nav.education,e.labels.nav.experience,e.labels.nav.projects,e.labels.nav.teaching,e.labels.nav.skills,e.labels.nav.reading,e.labels.nav.connect];o.forEach((s,c)=>{h[c]&&(s.textContent=h[c])}),d.forEach((s,c)=>{h[c]&&(s.textContent=h[c])});const m=document.getElementById("experience"),y=document.getElementById("projects"),b=document.getElementById("teaching"),p=document.getElementById("education"),E=document.getElementById("skills"),C=document.getElementById("reading"),S=document.getElementById("connect");m&&(m.innerHTML=O(e)),y&&(y.innerHTML=V(e)),b&&(b.innerHTML=J(e)),p&&(p.innerHTML=U(e)),E&&(E.innerHTML=Q(e)),C&&(C.innerHTML=Z(e)),S&&(S.innerHTML=ee(e)),m?.setAttribute("data-label",e.labels.headings.experience),y?.setAttribute("data-label",e.labels.headings.projects),b?.setAttribute("data-label",e.labels.headings.teaching),p?.setAttribute("data-label",e.labels.headings.education),E?.setAttribute("data-label",e.labels.headings.skills),C?.setAttribute("data-label",e.labels.headings.reading),S?.setAttribute("data-label",e.labels.headings.connect),document.querySelectorAll(".simulate-btn").forEach(s=>{s.addEventListener("click",()=>{s.closest(".project-card")?.querySelector(".sim-visual")?.classList.toggle("active")})}),j()},L=document.querySelector(".lang-switcher");v?.addEventListener("click",()=>{const t=f?.classList.contains("open");f?.classList.toggle("open",!t),L?.classList.toggle("open",!t),v?.setAttribute("aria-expanded",String(!t))});f?.querySelectorAll("button").forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-lang")||"en",n=document.documentElement.lang||"en";if(e===n){f.classList.remove("open"),L?.classList.remove("open"),v?.setAttribute("aria-expanded","false");return}localStorage.setItem("portfolio-lang",e),f.classList.remove("open"),L?.classList.remove("open"),v?.setAttribute("aria-expanded","false"),window.location.reload()})});document.addEventListener("click",t=>{if(!f||!v)return;const e=t.target;e instanceof Node&&(f.contains(e)||v.contains(e))||(f.classList.remove("open"),L?.classList.remove("open"),v.setAttribute("aria-expanded","false"))});const ie=localStorage.getItem("portfolio-lang")||"en";se(ie);const T=document.getElementById("theme-toggle"),M=T?.querySelector(".theme-icon"),D=()=>{const t=document.documentElement.classList.contains("light");M&&(M.textContent=t?"🌙":"☀️"),T?.setAttribute("aria-label",t?"Switch to dark mode":"Switch to light mode")};T?.addEventListener("click",()=>{document.documentElement.classList.toggle("light");const t=document.documentElement.classList.contains("light");localStorage.setItem("portfolio-theme",t?"light":"dark"),D()});D();const ne=window.matchMedia("(prefers-reduced-motion: reduce)").matches;if(!ne){const t=new WeakSet,e=()=>{document.querySelectorAll(".project-card, .meta-card").forEach(i=>{t.has(i)||(t.add(i),i.addEventListener("mousemove",a=>{i.style.transition="background 0.3s ease, border-color 0.3s ease";const o=i.getBoundingClientRect(),d=a.clientX-o.left,h=a.clientY-o.top,m=o.width/2,y=o.height/2,b=(h-y)/y*-4,p=(d-m)/m*4;i.style.transform=`perspective(600px) rotateX(${b}deg) rotateY(${p}deg)`,i.style.setProperty("--spot-x",`${d}px`),i.style.setProperty("--spot-y",`${h}px`)}),i.addEventListener("mouseleave",()=>{i.style.transition="transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), background 0.3s ease, border-color 0.3s ease",i.style.transform="",setTimeout(()=>{i.style.transition=""},650)}))})};e();const n=document.querySelector(".content");n&&new MutationObserver(()=>e()).observe(n,{childList:!0,subtree:!0});const r=new WeakSet,u=i=>{const a=i.textContent;if(!a||!a.trim())return;const o=a.length,d=400/o;let h=0;i.textContent="";const m=()=>{i.textContent=a.slice(0,++h),h<o&&setTimeout(m,d)};m()};document.querySelectorAll("section[id]").forEach(i=>{new MutationObserver((a,o)=>{if(i.classList.contains("section-revealed")){const d=i.querySelector("h2");d&&!r.has(d)&&(r.add(d),u(d)),o.disconnect()}}).observe(i,{attributes:!0,attributeFilter:["class"]})});const l=document.querySelectorAll(".side-nav a, .side-links a, .theme-toggle, .lang-toggle, .cta-btn");document.addEventListener("mousemove",i=>{l.forEach(a=>{const o=a.getBoundingClientRect(),d=o.left+o.width/2,h=o.top+o.height/2,m=i.clientX-d,y=i.clientY-h,b=Math.sqrt(m*m+y*y);if(b<50){const p=(1-b/50)*.35;a.style.transform=`translate(${m*p}px, ${y*p}px)`}else a.style.transform&&(a.style.transform="")})})}
